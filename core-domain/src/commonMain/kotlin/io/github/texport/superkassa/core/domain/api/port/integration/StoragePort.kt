...
interface StoragePort : DeliveryTaskStore {
    ...
    /**
     * Запоминает ссылку на чек в ОФД, пришедшую в ответе на документ.
     *
     * По ссылке печатается QR-код проверки чека, и нужна она не только
     * в момент ответа: чек печатают и присылают повторно позже. Хранилище,
     * которое ссылку не хранит, отвечает `false` — так было до появления
     * метода, и вызывающий от этого не падает.
     *
     * @param documentId идентификатор документа.
     * @param receiptUrl ссылка на чек, как её прислал ОФД.
     * @return `true`, если ссылка сохранена.
     */
    fun saveReceiptUrl(documentId: String, receiptUrl: String): Boolean = false
