...
class TestStoragePort : StoragePort {
    ...
    private val queueTasks = mutableListOf<QueueTask>()
    ...
    override fun saveReceipt(request: ReceiptRequest, documentId: String, shiftId: String, createdAt: Long): Boolean {
        ...
        val snapshot =
            FiscalDocumentSnapshot(
                ...
                shiftId = shiftId,
                // Тип документа называет операцию, как его пишет настоящее
                // хранилище. Пока двойник писал «CHECK», пересчёт в тестах
                // видел чеки, а в работе — нет.
                docType = ReceiptDocumentTypes.of(request.operation),
                docNo = null,
                shiftNo = shift?.shiftNo,
                createdAt = createdAt,
                // Итог хранится в тиынах, как и в настоящем хранилище.
