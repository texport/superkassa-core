...
/**
 * Интерфейс-маркер для поддержки спецификации Jakarta Bean Validation в общем коде.
 *
 * ### Назначение и обоснование:
 * Объявлен как ожидаемый (`expect`) интерфейс для обеспечения совместимости типов в KMP.
 * Используется в сигнатурах валидационных аннотаций в качестве параметра `payload`
 * (например, `val payload: Array<KClass<out Payload>> = []`), что позволяет прикреплять
 * пользовательские метаданные (например, уровни критичности ошибок) к ограничениям.
 *
 * ### Расширение и реализация:
 * Разработчики могут объявлять собственные маркерные классы в общем коде (`commonMain`), наследуя их от [Payload]:
 * ```kotlin
 * object Severity {
 *     class Warning : Payload
 *     class Error : Payload
 * }
 * ```
 *
 * ### Особенности платформ:
 * - **JVM / Android**: Связывается через `typealias` напрямую с оригинальным `jakarta.validation.Payload`,
 *   благодаря чему валидаторы на сервере (Hibernate Validator/Spring) могут считывать эти метаданные.
 * - **iOS**: Компилируется в локальный маркерный интерфейс для совместимости типов. Поскольку на iOS отсутствует
 *   автоматический движок Jakarta Bean Validation, для выполнения валидации по этим метаданным на стороне клиента
 *   потребуется написать или подключить кроссплатформенный валидатор (например, Konform), считывающий аннотации.
 */
