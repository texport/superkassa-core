...
interface SuperkassaApi : PrintApi {
    ...
    /**
     * Закрыть смену автоматически, если пора (настройка кассы «автозакрытие»).
     *
     * Закрывает открытую смену, когда до предела в сутки осталось меньше
     * получаса или он пройден. Кассир не нужен; правила и исход те же,
     * что у [closeShift]: отказ БФД смену не закрывает, и отклонённый
     * Z-отчёт автоматически не повторяется. Зовёт хозяин процесса по часам.
     *
     * @param kkmId ID ККМ.
     * @return Итог закрытия или `null`, если закрывать сейчас нечего.
     */
    @Throws(Exception::class)
    fun autoCloseShift(kkmId: String): ReportResponse?
